...
https.get('https://maps.googleapis.com/maps/api/geocode/json?' + search, function(res){
  var data = '';
  ...
  res.on('end', function(){
    ...
    var options = querystring.stringify({ units: 'si', lang: 'pt' })
      https.get('https://api.forecast.io/forecast//' + location.lat +',' + location.lng + '?' + options, function(resForecast){
          var data = '';
